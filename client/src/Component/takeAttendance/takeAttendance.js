    ...
    render() {
        ...
        return (
            <div className="attendance-container">
                ...
                {admin && <div className="attendance-header">Input Student Matric No.</div>}
                {admin && <input
                    className="attendance-input"
                    type="text"
                ...
                <div className="attendance-btn" onClick={!admin ? this.onBtnClick : this.onAdminClick}>
                    Submit
                </div>
            </div>
        );
        ...
        this.generateDescriptors()
            .then(res => {
                ...
                fetch("http://localhost:5000/api/attendance/update/", {
                    ...
                    .then(res => {
                        if (!res) {
                            alert("Photo does not match you. Please try again or approach administrator")
                        } else if (res === true) {
                            alert("Attendance has been recorded")
                        }
                    });
            })
            .catch(e => alert("Photo not valid. Please retake and try again or approach administrator"));
    };
